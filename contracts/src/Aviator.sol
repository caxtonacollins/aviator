...
/**
 * @title AviatorGame
 * @dev Crash game with off-chain randomness (server-side crash calculation)
 * @notice Players bet before each round, multiplier increases until crash
 */
contract AviatorGame is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public constant FLYING_DURATION = 15 seconds;
    
    // Game State
    enum GamePhase { BETTING, FLYING, CRASHED }
    ...
    event RoundStarted(uint256 indexed roundId, uint256 startTime, bytes32 serverSeedHash);
    event BetPlaced(uint256 indexed roundId, address indexed player, uint256 amount);
    event CashOut(uint256 indexed roundId, address indexed player, uint256 amount, uint256 multiplier, uint256 payout);
    ...
    event RoundSettled(uint256 indexed roundId, uint256 totalBets, uint256 totalPayouts);
     ...
     */
    function placeBet() external payable nonReentrant whenNotPaused {
        ...
        if (roundBets[currentRoundId][msg.sender].amount > 0) revert BetAlreadyPlaced();
        ...
        currentRound.totalBets += msg.value;
    ...
    function cashOut(uint256 multiplier) external nonReentrant whenNotPaused {
        ...
        emit CashOut(currentRoundId, msg.sender, bet.amount, multiplier, payout);
    }
     ...
     */
    function startFlying(bytes32 serverSeedHash) external onlyServerOperator whenNotPaused {
        if (currentRound.phase != GamePhase.BETTING) revert WrongGamePhase();
     ...
     */
    function crashRound(uint256 crashMultiplier, string calldata serverSeed) 
        external 
        onlyServerOperator 
        whenNotPaused 
    {
        if (currentRound.phase != GamePhase.FLYING) revert WrongGamePhase();
        if (crashMultiplier < MIN_MULTIPLIER || crashMultiplier > MAX_MULTIPLIER) {
            revert InvalidCrashMultiplier();
        }
        
        // Verify server seed matches hash (provably fair)
    ...
    function _getCurrentMultiplier() internal view returns (uint256) {
        ...
        // multiplier = 1.0 + (time^1.5 / 5)
        // Scaled by 100 for precision
        uint256 timeSquared = elapsed * elapsed;
        uint256 growth = (timeSquared * 20) / 1000; // Adjust growth rate
     ...
     */
    function getMyBet() external view returns (
        uint256 amount,
     ...
     */
    function getRoundHistory(uint256 roundId) external view returns (
        uint256 id,
        uint256 crashMultiplier,
    ...
    function emergencyWithdraw() external onlyOwner {
        ...
        require(currentRound.phase == GamePhase.BETTING && currentRound.totalBets == 0, "Active round exists");
